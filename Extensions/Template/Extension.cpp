...
Extension::Extension(RD *rd, SerializedED *SED, createObjectInfo *COB) : rd(rd), rh(rd->rHo.hoAdRunHeader), Runtime(rd)
{
	...
	LinkAction(0, create_deck);
	LinkAction(1, shuffle_deck_small);
	LinkAction(2, shuffle_deck_medium);
	LinkAction(3, shuffle_deck_large);
	LinkAction(4, split_deck_clockwise);
	LinkAction(5, split_deck_counterclockwise);
	LinkAction(6, give_single_card);
	LinkAction(7, shuffle_deck_custom);
	LinkAction(8, AddQueue);
	LinkAction(9, RemoveQueue);
	LinkAction(10, give_multiple_cards);
	LinkAction(11, clear_all_decks);
	LinkAction(12, Loop_Through_Deck);
	

	LinkExpression(0, reveal_Deck_CardNumber);
	LinkExpression(1, reveal_Deck_CardName);
	LinkExpression(2, reveal_Deck_CardSuit);
	LinkExpression(3, reveal_Deck_Card);
	LinkExpression(4, convertCardID_Name);
	LinkExpression(5, convertCardID_Suit);
	LinkExpression(6, convertCardID);
	LinkExpression(7, deck_size);
	LinkExpression(8, QueueIndex);
	LinkExpression(9, QueueSize);
	LinkExpression(10, LastLoopedCard);
	LinkExpression(11, LastLoopedDeck);
	LinkExpression(12, LastLoopedIndex);

	LinkCondition(0, InvalidDeckID);
	...
	LinkCondition(6, SpecificDeckEmptied);
	LinkCondition(7, SpecificDeckDecreased);
	LinkCondition(8, SpecificDeckIncreased);
	LinkCondition(10, SpecificDeckShuffled);
	LinkCondition(11, Loop_Through_Deck_Immediate);
	
	LinkCondition(12, CardClub);
	LinkCondition(13, CardDiamond);
	LinkCondition(14, CardHeart);
	LinkCondition(15, CardSpade);
	LinkCondition(16, CardSuit);
	LinkCondition(17, CardValue);
	LinkCondition(18, DeckSetExists);
